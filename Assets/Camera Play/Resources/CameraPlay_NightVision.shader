...
uv = lerp(uv,screenDistort(uv),_Distortion*_flashy);
col = tex2D(_MainTex,uv).rgb;
float3 colold = col;
col.rgb+=lerp(0.0,rand(uv*floor(_TimeX*8))/2,_Noise);
float vignette=smoothstep( 0.15+_Vignette,0.5+_Vignette, length(float2(0.5,0.5)-uvst));
col -=(vignette*_Vignette_Alpha);
float vignette2=smoothstep( 0.25+_Vignette,0.0+_Vignette, length(float2(0.5,0.5)-uvst));
col +=(vignette2*_Light2);
col +=_Intensity;
float3 colm=col;
col *= float3(0.55,1.55+_Greenness/4,0.55)*_flashy;
col *= lerp(1.0,0.8+0.1*sin(10.0*_TimeX+uv.y*300.0*_Light*_flashy),_Line);
col.r+=_Color_R;
col.g+=_Color_G;
col.b+=_Color_B;
uv = i.texcoord;
uv.x=uv.x/0.72;
float dif=_Size-_Smooth*_flashy;
